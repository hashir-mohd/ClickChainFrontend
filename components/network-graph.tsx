...
export default function NetworkGraph({ logs, onSelectLog }: NetworkGraphProps) {
  ...
  }, [isPlaying, graphData.nodes.length])
  ...
  // Filter nodes based on timeline position and event types
  const filteredGraphData = useMemo(() => {
    if (!graphData.nodes.length) return { nodes: [], links: [] }

    // Get the time range of all events
    const allTimestamps = graphData.nodes.filter((node) => node.timestamp).map((node) => node.timestamp)
    ...
    // Filter nodes based on timestamp and event types
    const filteredNodes = graphData.nodes.filter((node) => {
      // Always include domain nodes
      if (node.type === "domain") return true

      // Filter by timestamp
      if (node.timestamp && node.timestamp > cutoffTime) return false

      // Filter by event type
      if (filteredEventTypes.length > 0 && node.eventType && !filteredEventTypes.includes(node.eventType)) return false
    ...
    // Filter links based on whether both source and target nodes are included
  ...
  }, [graphData, timelinePosition, filteredEventTypes])
  ...
  const renderNode = (node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    const { x, y, val, color, icon, id } = node
    const size = val || 5
    ...
    if (icon === "globe") {
      ...
      if (node.method === "GET") ctx.fillText("GET", x, y)
      else if (node.method === "POST") ctx.fillText("POST", x, y)
      else if (node.method === "PUT") ctx.fillText("PUT", x, y)
      else if (node.method === "DELETE") ctx.fillText("DEL", x, y)
      else if (node.method === "OPTIONS") ctx.fillText("OPT", x, y)
      else ctx.fillText("API", x, y)
    } else {
      // Default icon
      ctx.fillText("•", x, y)
    }
      ...
      <div className="flex-1 relative">
        {filteredGraphData.nodes.length > 0 ? (
          <ForceGraph2D
            ...
            height={dimensions.height - 80} // Subtract height of controls
